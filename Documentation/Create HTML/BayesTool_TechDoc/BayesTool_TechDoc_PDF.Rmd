---
title: "In-Season Bayesian Risk Assessment Tool"
...
The developers of the Bayesian Risk Assessment Tool and authors of this document are Ben Staton and Matt Catalano. Ben is a Graduate Student Researcher and USFWS Pathways Quantitative Ecologist, who has studied the statistical aspects of Chinook salmon assessment and management issues in the Kuskokwim River since 2014. The statistical framework for the Tool was developed jointly by Ben and Matt, who is the Principle Investigator of the Quantitative Fisheries Laboratory at Auburn University and has studied salmon assessment methods in Alaska since 2009.

# Contributors{-}

Many people have given incredibly useful feedback on the Tool and on its utility for aiding in management discussions (listed alphabetically):

*  Dr. Bill Bechtol, statistical consultant for the Kuskokwim River Inter-Tribal Fish Commission
*  Dr. Lew Coggins, biology program supervisor, U.S. Fish and Wildlife Service, Yukon Delta National Wildlife Refuge
*  Zach Liller, Research Coordinator for the Arctic-Yukon-Kuskokwim Region, Alaska Department of Fish and Game, Commercial Fisheries Division
*  Nick Smith, Kuskokwim Area Fisheries Research Biologist, Alaska Department of Fish and Game, Commercial Fisheries Division
*  Kuskokwim area fisheries managers, from the Alaska Department of Fish and Game, Kuskokwim River Inter-Tribal Fish Commission, and the U.S. Fish and Wildlife Service

# Purpose{-}

This document is intended to be a companion to the _In-Season Chinook Salmon Bayesian Risk Assessment Tool_ (hereafter, "Bayes' Tool" or simply "the Tool") that describes the motivations for its development and the technicalities of what it does. While this document highlights some of the features of the Tool, it does not provide insights about how to use it. It is the hope of the developers that the Tool is intuitive to use on its own after being introduced to it, but to address any questions or confusion, detailed instructions are documented at this URL: <https://bstaton.shinyapps.io/BayesTool_UserMan/>.

**The Tool was developed for use for Kuskokwim River Chinook salmon only**. It was not developed for any other stock in the Kuskokwim River or in the state of Alaska. This is because the historical data used in the Tool are from the Kuskokwim River Chinook salmon stock. It is possible that similar tools could be developed in the future for other stocks, but at present this is not the case. Hereafter, all references to salmon, fish, runs, escapement, and harvest are about the drainage-wide stock of Chinook salmon in the Kuskokwim River.
...
Consider the simple case in which managers are interested in only three run size hypotheses: 
...
We realize that these categories are not appropriate when taken in context of all historical run sizes that have occurred in the Kuskokwim River, but this example is for illustration purposes only, so please forgive us for this simplification for the time being. Using the pre-season forecast, managers assign the following prior probabilities:
...
```{r Categorized Regression Plot, fig.height = 4.5, fig.width = 4.5, fig.align = "center"}
par(xaxs = "i", yaxs = "i", mar = c(3,3,0.5,0.5), cex.axis = 0.8, cex.lab = 1, mgp = c(1.4,0.4,0), tcl = -0.25)
...
axis(side = 2, at = seq(0, 180000, 20000), labels = seq(0, 180, 20), las = 2, lwd = 2)
axis(side = 1, at = seq(0, 900, 100), labels = seq(0, 900, 100), las = 1, lwd = 2)
...
text(x = c(150, 450, 750), y = usr[3] + ydiff * 0.03, cex = 1,
     labels = c("BTF: Small", "BTF: Medium", "BTF: Large"), col = "grey", font = 4)
text(x = usr[1] + xdiff * 0.03, y = c(30000, 90000, 150000), cex = 1, srt = 90,
...
*  In all `r N_lrg` years when the run was actually large, the observation of $data_{medium}$ was made `r lrg_med` times. To calculate $P(data_{medium}|H_{large})$, we take `r lrg_med` and divide it by `r N_lrg` and obtain `r round(lrg_med/N_lrg, 2)`. 
...
Now, this is illustrative example is just that: an illustration. Although the data plotted in the figure are real (and exactly those used by the Tool) and the Bayesian calculations are correct, this example suffers from two primary downfalls that prevent it from being the way the Tool actually works. First, the borders of the categories (e.g., small runs are less than 60,000) are totally arbitrary. Attentive readers will notice that one run was on the border of being small and medium. In reality, run size is not categorized this way, but instead numbers of fish are desired for management purposes. That is to say, we need to consider **many** different hypotheses, each one representing a unique run size (e.g., that the total run will be 100,000 fish, 101,000 fish, etc. and everywhere in between). The second issue is that there is substantial uncertainty regarding which category the BTF data fall into for much of the season, which is due to annual variability in run timing (i.e., the peak of the run happens at different times in the summer each year). On June 22, approximately 50% of the run is complete in an average timing year, but could be as low as 30% complete in a late year and as high as 70% complete in an early year. These two issues introduce mathematical and statistical complexities into the application of Bayes' Theorem for this updating problem, and are treated in full in the sections that follow. 
...
Note that for these calculations, the conditions $0 < \hat{\mu}_{p_d} < 1$ and $0 < \hat{\sigma}_{p_d}^2$ must hold true. The only consequence of this is that the Tool cannot be ran on August 24^th^, as that is the last day of the BTF index and all $p_{d,t} = 1$.
...
where the subscript $b$ denotes MC samples. Finally, we obtain MC samples of $\widehat{EOS}_{d,b}$: 
...
The MC samples $\widehat{EOS}_{d,b}$ are predictions of the index of total run size past the BTF. In order to obtain predictions of the run size past the BTF, simple linear regression is used on the data presented in the figure shown in the example of Bayes' Theorem. That is, we fit a model of the form:
                 
\begin{equation}
  \log(N_{vuln,t}) = \beta_0 + \beta_1\log(EOS_t)+\varepsilon_t, \varepsilon_t \sim Normal(0, \sigma_{\varepsilon}),
  (\#eq:fitreg)
\end{equation}
                 
to all historical years and obtain the estimates $\hat{\beta}_0$, $\hat{\beta}_1$, $\hat{\sigma}_{\varepsilon}$, and $\hat{\Sigma}$. The last of these is the estimated variance-covariance matrix of the estimated regression coefficients $\left(\hat{\beta}_0, \hat{\beta}_1\right)$, and is used to propagate regression uncertainty in the fitted relationship forward to prediction uncertainty as follows. First, MC samples of the coefficients are obtained:
...
\begin{equation}
  \log(\hat{H}_{DS,d,b}) \sim Normal\left(-\frac{\hat{\sigma}^2_{\hat{H}_{DS,d,b}}}{2},
  \hat{\sigma}_{\hat{H}_{DS,d,b}}\right),
  (\#eq:harv)
\end{equation}  
...
One approach to using the Tool would be to set a critical escapement limit $L$ such that managers would deem the event $S \leq L$ undesirable. Managers can then select a maximum probability of the occurrence of the undesirable event $S \leq L$ they are willing to live with. In the harvest control systems literature [@ShertzerCite; @PragerCite], this maximum probability is known as $P^*$ and is an expression of maximum acceptable risk (i.e., risk tolerance). The option in the Tool entitled "Risk Analysis: Choose Harvest Objective" uses the $P^*$ notion to pick the maximum allowable harvest that would result in $P(S \leq L) < P^*$. Note that it is entirely possible to obtain the same suggested target harvest using two different levels of $P^*$ and $L$.
...
We used Program R [@RCite] to perform all calculations used in the Tool. R is a coding platform for statistical computation, and is widely used among scientists across many disciplines. The Tool was developed in R using a package called `shiny` [@shinyCite], which allows R code to be integrated into a web interface. This document was produced using the packages `rmarkdown` [@rmarkdownCite], `bookdown` [@bookdownCite], and `knitr` [@knitrCite].
